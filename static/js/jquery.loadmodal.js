...
(function($) {
    ...
    $.loadmodal = function(options) {
        ...
        options = $.extend(true, {
            ...
            size: 'modal-sm',                                   // CSS class(es) to specify the dialog size ('modal-lg', 'modal-sm', '').  The default creates a large dialog.  See the Bootstrap docs for more info.
        ...
        options.ajax.success.unshift(function(data, status, xhr) { // unshift puts this as the first success method
			...
			//div.find('.modal-dialog').css('margin', '150px auto');
			div.find('.modal-dialog').css('position', 'absolute');
			div.find('.modal-dialog').css('left', '50%');
			div.find('.modal-dialog').css('top', '50%');
			div.find('.modal-dialog').css('margin-left', '-220px');
			div.find('.modal-dialog').css('margin-top', '-169px');
